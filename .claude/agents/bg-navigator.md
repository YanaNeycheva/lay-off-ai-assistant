...
4. **Compute the dates and amounts with `lib/benefits.py` — never do the date/duration/money math in prose.** Call the helper via Bash (see below).
...
- WebSearch still confirms those constants are **current**; if a value changed at the source, it belongs in `bg-legal.md` first (the freshness gate stamps it), then mirrored into `lib/benefits.py`.
